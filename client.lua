Citizen.CreateThread(function()
	Wait(1000)
	TriggerServerEvent("wcukReports:FetchFeedbackTable")
end)
...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['wcukNotify']:Alert("REPORT", "You can't report so quickly!", 10000, 'warning')
	end
end, false)
...
RegisterNetEvent('wcukReports:NewFeedback')
AddEventHandler('wcukReports:NewFeedback', function(newFeedback)
	if staff then
		FeedbackTable[#FeedbackTable+1] = newFeedback
exports['wcukNotify']:Alert("REPORT", "New Report Some one needs help!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
RegisterNetEvent('wcukReports:FetchFeedbackTable')
AddEventHandler('wcukReports:FetchFeedbackTable', function(feedback, admin, oneS)
	FeedbackTable = feedback
	staff = admin
	oneSync = oneS
end)

RegisterNetEvent('wcukReports:FeedbackConclude')
AddEventHandler('wcukReports:FeedbackConclude', function(feedbackID, info)
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['wcukNotify']:Alert("REPORT", "Report successfully sent to the STAFF!", 20000, 'success')
		
		local feedbackInfo = {subject = data.subject, information = data.information, category = data.category}
		TriggerServerEvent("wcukReports:NewFeedback", feedbackInfo)
	...
	elseif data.action == "assistFeedback" then
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				TriggerServerEvent("wcukReports:AssistFeedback", data.feedbackid, true)
			else
				...
				if playerOnline then
					...
					exports['wcukNotify']:Alert("REPORT", "That player is no longer in the server!", 20000, 'error')
				end
			end
		end
	elseif data.action == "concludeFeedback" then
		...
		if feedbackInfo then
			if feedbackInfo.concluded ~= true or canConclude then
				TriggerServerEvent("wcukReports:FeedbackConclude", feedbackID, canConclude)
				exports['wcukNotify']:Alert("FEEDBACK", "Feedback #"..feedbackID.." concluded!", 20000, 'success')
			end
		end
	end
end)