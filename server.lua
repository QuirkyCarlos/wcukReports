...
AddEventHandler("wcukReports:NewFeedback", function(data)
	...
	TriggerClientEvent("wcukReports:NewFeedback", -1, newFeedback)
...
AddEventHandler("wcukReports:FetchFeedbackTable", function()
	local staff = hasPermission(source)
	if staff then
		staffs[source] = true
		TriggerClientEvent("wcukReports:FetchFeedbackTable", source, FeedbackTable, staff, oneSync)
	end
end)

-------------------------- ASSIST FEEDBACK

RegisterNetEvent("wcukReports:AssistFeedback")
AddEventHandler("wcukReports:AssistFeedback", function(feedbackId, canAssist)
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				TriggerClientEvent('wcukNotify:Alert', source, "REPORT", "You are assisting FEEDBACK #"..feedbackId.."!", 20000, 'info')
				TriggerClientEvent('wcukNotify:Alert', id, "REPORT", "An admin arrived!", 20000, 'info')

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				TriggerClientEvent('wcukNotify:Alert', id, "REPORT", "That player is no longer in the server!", 20000, 'error')
			end
			if not FeedbackTable[feedbackId].concluded then
				FeedbackTable[feedbackId].concluded = "assisting"
			end
			TriggerClientEvent("wcukReports:FeedbackConclude", -1, feedbackId, FeedbackTable[feedbackId].concluded)
		end
	end
end)
...
AddEventHandler("wcukReports:FeedbackConclude", function(feedbackId, canConclude)
	if staffs[source] then
